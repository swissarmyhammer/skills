...
- **Trace dependents** — `op: "get callgraph"` with `direction: "inbound"` to find a symbol's callers. When a change touches a shared symbol's signature, `op: "get blastradius"` on the file reveals the wider set of callers, downstream consumers, tests, and transitive dependencies; if it's large, consider scoping the change more narrowly. Blast radius is built from LSP call edges, so an empty `edges: []` means LSP isn't ready, not "no impact" — fall back to inbound callgraph and don't gate on it.
...
- **Skipping dependent analysis on shared-symbol changes** — not checking inbound callers (and blast radius for shared/public symbols) leads to missed downstream work and surprise breakage.
