...
If the text is empty, use the current branch. If the text is not empty, read it as a PR number, a branch, a run or pipeline id, or `wait`/`fix`. It can hold a target and a mode together (e.g. `123 fix`). If nothing matches, use it as a branch name.
